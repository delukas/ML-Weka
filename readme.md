...
## Hinweise
Wenn ein Modell in 'src/main/resources' vorhanden ist, kann man dieses über die UI einfach durch `Modell laden` schnell laden. Wenn das Modell neu trainiert werden soll, muss das vorhandene Modell in diesem Ordner gelöscht werden, die ARFF Dateien dort verfügbar sein, die UI neugestartet werden und dann durch `Modell trainieren` trainiert werden. (Dauert deutlich länger). Danach ist das Modell sofort in der UI verfügbar und wird genau diesem Ort gespeichert. Nach dem Trainieren wird in der Konsole automatisch eine Evaluation durchgeführt.